...
const budgetRecords = {
  ...
  calculateTotal: function (data, sth) {
    // add your code here
    ...
    if (elements.selectOption.value === "inc") {
      ...
      // counta += 1;
      // console.log(counta)
      this.incomeTotal += Number(data - sth);
      ...
      if (this.incomeTotal == 0 && this.expenseTotal == 0) {
        ...
        for (let i = 0; i < counta; i++) {
          document.getElementById(`ind-${i}`).innerHTML = `${
            (elements.budgetValue.value / this.expenseTotal) * 100
          }%`;
        }
      } else if (this.incomeTotal > 0 && this.expenseTotal > 0) {
      ...
      <div class="right clearfix">
        ...
        this.expenseTotal += Number(data - sth);
        this.remainingBudget = this.incomeTotal - this.expenseTotal;
        ...
        if (this.incomeTotal == 0 && this.expenseTotal == 0) {
          ...
          for (let i = 0; i <= countb; i++) {
            ...
            indExpa.innerHTML = `${((vera / this.incomeTotal) * 100).toFixed(
              1
            )}%`;
          }
        }
...
elements.addButton.addEventListener("click", () => {
  ...
  if (recordType === "inc") {
    ...
    budgetRecords.calculateTotal(value, 0);
    console.log("Yes, income added");
  } else {
    if (elements.descBox.value === "" || elements.budgetValue.value === "") {
      budgetRecords.showAlert("Please fill in all fields", "info");
      return;
...
elements.incomeList.addEventListener("click", (event) => {
  ...
  const val1 =
    event.target.parentElement.parentElement.parentElement.childNodes[1]
      .innerHTML;
  budgetRecords.removeItem(event.target);
  budgetRecords.calculateTotal(value, val1); //specific cause there are many buttons with same class
});
